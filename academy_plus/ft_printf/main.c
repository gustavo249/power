...
/*   Updated: 2016/01/11 19:33:23 by rcrisan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_width(char *choped)
{
	...
	while (i >= 0)
	{
		if (choped[i] == '0' && !isdigit(choped[i - 1]))
			i--;
		else if (!isdigit(choped[i]))
			i--;
		else
			break;
	}
	while (isdigit(choped[i]) && i >= 0)
		width[k++] = choped[i--];
	width[k] = '\0';
	ft_strrev(width);
	return (atoi(width));
...
char	*convert_strings(t_mod *data, va_list *arg)
{
	...
	str = (char*)malloc(sizeof(str) * 200);
	if (data->specifier == 's')
		str = va_arg(*arg, char*);
	return (str);
}
